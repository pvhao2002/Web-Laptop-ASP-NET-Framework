...
(function ($) {
    ...
    /*----------------------------------
        27. Visitor Fake Message
      ------------------------------------*/
    var userLimit = parseInt($(".userViewMsg").attr("data-user"), 10),
        userTime = $(".userViewMsg").attr("data-time");
    // random from 1 to 10
    var storeUser = Math.floor(Math.floor(Math.random() * 30) + 1 + userLimit);
    $(".uersView").text(storeUser);
    setInterval(function () {
        storeUser += Math.floor(
            Math.floor(Math.abs(Math.random()) * 300) + 1 + userLimit
        );
        $(".uersView").text(formatNumber(storeUser));
    }, userTime);
